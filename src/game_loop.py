...
instrucciones_channel = pygame.mixer.Channel(5)  # Usar un canal dedicado
instrucciones_sound = pygame.mixer.Sound("assets/sounds/instrucciones.mp3")
...
def draw_help_box(screen, instructions, font):
	# Reproducir el audio solo si no está sonando en el canal dedicado
	if not instrucciones_channel.get_busy():
		instrucciones_channel.play(instrucciones_sound)
...
def mostrar_ranking_modal(screen, fondo_surface=None):
    ...
    try:
        with open("data/scores.csv", "r", encoding='utf-8') as f:
            for row in csv.reader(f):
                print(f"Leyendo fila: {row}")  # Debug
                # Ignorar líneas vacías o mal formateadas
                if not row or len(row) < 3:
                    continue
                try:
                    nombre, dificultad, puntaje = row
                    print(f"Procesando: {nombre}, {dificultad}, {puntaje}")  # Debug
                    ...
                    print(f"Error procesando fila: {e}")  # Debug
                    continue
    ...
    for key in ranking:
        ...
        print(f"Ranking {key}: {ranking[key]}")  # Debug
...
def show_difficulty_menu(screen):
	...
	while not selected_mode:
		...
		# Botón de cambio de usuario en la parte inferior izquierda
		change_user_btn = pygame.Rect(20, SCREEN_HEIGHT - 70, 300, 48)
		pygame.draw.rect(screen, (70, 200, 255), change_user_btn, border_radius=12)
		pygame.draw.rect(screen, (0,0,0), change_user_btn, 4, border_radius=12)
		change_user_font = pygame.font.Font("assets/fonts/PressStart2P-Regular.ttf", 16)
		change_user_text = change_user_font.render("Cambiar Usuario", True, (0,0,0))
		screen.blit(change_user_text, (change_user_btn.centerx-change_user_text.get_width()//2, change_user_btn.centery-change_user_text.get_height()//2))
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.MOUSEBUTTONDOWN:
				mx, my = pygame.mouse.get_pos()
				if change_user_btn.collidepoint(mx, my):
					return "CHANGE_USER"  # Señal para cambiar de usuario
...
def run_game():
    ...
    while running:
        ...
        if result == "CHANGE_USER":
            # Solo volver a pedir el nombre si se hizo clic en el botón de cambiar usuario
            nombre_jugador = get_player_name(screen, fondo_surface)
            continue
        if len(result) == 3:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if not state["game_over"]:
                        ...
                        if menu_btn.collidepoint(mx, my):
                            in_game = False  # Volver al menú de dificultad
                            break
            ...
            if not state["game_over"]:
                ...
                if modo_ia:
                    ...
                    if not state.get("score_saved", False):
                        print(f"Guardando puntuación: {nombre_jugador}, {difficulty}, {state['score']}")  # Debug
                        try:
                            # Primero leer todas las puntuaciones existentes
                            scores = []
                            try:
                                with open("data/scores.csv", "r", encoding='utf-8') as f:
                                    for row in csv.reader(f):
                                ...
                                pass  # El archivo no existe, se creará nuevo
                            
                            # Buscar si el usuario ya tiene una puntuación en este modo
                            usuario_existe = False
                            for i, score in enumerate(scores):
                                if score[0] == nombre_jugador and score[1] == difficulty:
                                    # Si el nuevo puntaje es mayor, actualizar
                                    if state['score'] > int(score[2]):
                                        scores[i] = [nombre_jugador, difficulty, str(state['score'])]
                                        print(f"Actualizando puntaje de {nombre_jugador} en {difficulty}")  # Debug
                                    usuario_existe = True
                                    break
                            
                            # Si el usuario no existe en este modo, agregar nueva puntuación
                            if not usuario_existe:
                                scores.append([nombre_jugador, difficulty, str(state['score'])])
                                print(f"Agregando nueva puntuación para {nombre_jugador} en {difficulty}")  # Debug
                            
                            # Ordenar las puntuaciones por dificultad y puntaje
                            scores.sort(key=lambda x: (x[1], -int(x[2])))
                            ...
                            print("Puntuación guardada correctamente")  # Debug
                            state["score_saved"] = True
                        except Exception as e:
                            print(f"Error al guardar la puntuación: {e}")  # Debug
